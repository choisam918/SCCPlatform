...
function showDefaultCards() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (cardElement) {
            ...
            const cardCategory = cardElement.querySelector('.card-category');
            
            if (cardIcon) cardIcon.textContent = '✨';
            if (cardText) cardText.textContent = '準備好抽卡了嗎？';
            if (cardCategory) cardCategory.textContent = '鼓勵';
        }
    }
    // 隱藏第二行的卡片
    for (let i = 3; i < 6; i++) {
...
function displayRecordPage(pageIndex) {
    ...
    cards.forEach((card, index) => {
        ...
        if (cardFront) {
            ...
            // 使用純色背景而非漸變以提升性能
            // 將顏色轉換為 rgba 格式用於淡色背景
            const rgb = hexToRgb(cardColor);
            if (rgb) {
                cardFront.style.background = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.08)`;
            } else {
        ...
        if (cardBack) {
            // 使用純色而非漸變以提升性能
            cardBack.style.background = cardColor;
        }
...
function updateNavigationButtons(totalPages) {
    ...
    console.log('更新導航按鈕，totalPages:', totalPages, 'currentPageIndex:', currentPageIndex);
    ...
    if (totalPages > 1) {
        ...
        if (prevBtn) {
            ...
            console.log('上一頁按鈕:', prevBtn.style.display, 'disabled:', prevBtn.disabled);
        }
        if (nextBtn) {
            nextBtn.style.display = 'flex';
            nextBtn.style.visibility = 'visible';
            nextBtn.disabled = currentPageIndex >= totalPages - 1;
            console.log('下一頁按鈕:', nextBtn.style.display, 'disabled:', nextBtn.disabled);
        }
        if (pageIndicator) {
