...
function handleFiles(files) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        for (var j = 0; j < supportedExtensions.length; j++) {
            if (fileName.endsWith(supportedExtensions[j])) {
                isValid = true;
                break;
            }
        }
        
        if (!file.name || !isValid) {
            alert('檔案「' + file.name + '」不是支援的檔案格式（.py），已跳過。');
            continue;
        }
        ...
        if (file.size > maxSize) {
            ...
            alert('檔案「' + file.name + '」超過 ' + maxSizeMB + 'MB 限制，已跳過。');
            continue;
        }
    ...
    validFiles.forEach(function(file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                fileData.isBinary = false;
                fileData.lineCount = content.split('\n').length;
                
                // 調試：檢查檔案內容是否正確
                if (!fileData.content) {
                    console.error('檔案內容為空:', file.name, fileData);
                    alert('檔案「' + file.name + '」內容為空，無法儲存。請確認檔案未損壞。');
                    return;
                }
                
                console.log('檔案處理成功:', file.name, '類型:', fileType, '內容長度:', fileData.content.length);
