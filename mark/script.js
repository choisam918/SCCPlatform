    ...
    requestAnimationFrame(() => {
        ...
        if (cards.length > 0 && cards.length > 10) {
            ...
            setTimeout(() => {
                ...
                if (firstCard) {
                    const cardHeight = firstCard.offsetHeight || firstCard.getBoundingClientRect().height;
                    const gap = 8; // 與 CSS 中的 gap 一致
                    const padding = 4; // 與 CSS 中的 padding 一致
                    const rowsToShow = 2; // 固定顯示2行
                    const maxHeight = (cardHeight * rowsToShow) + (gap * (rowsToShow - 1)) + (padding * 2);
