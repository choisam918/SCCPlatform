...
function renderCardView() {
    ...
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            ...
            let gap = parseFloat(containerStyle.gap) || 12;
            let padding = parseFloat(containerStyle.paddingLeft) + parseFloat(containerStyle.paddingRight) || 12;
            ...
            // 手機版使用較小的 gap 和 padding
            if (isMobilePortrait || window.innerWidth <= 768) {
                gap = parseFloat(containerStyle.gap) || (isMobilePortrait ? 4 : 6);
                padding = parseFloat(containerStyle.paddingLeft) + parseFloat(containerStyle.paddingRight) || 8;
            }
            if (isMobileSmall) {
                gap = parseFloat(containerStyle.gap) || 4;
                padding = parseFloat(containerStyle.paddingLeft) + parseFloat(containerStyle.paddingRight) || 8;
            }
            ...
            // 設置容器高度為2行（僅在非手機橫屏時限制高度）
            let paddingTop = parseFloat(containerStyle.paddingTop) || 6;
            let paddingBottom = parseFloat(containerStyle.paddingBottom) || 6;
            
            // 手機版豎屏使用較小的 padding
            if (isMobilePortrait) {
                paddingTop = parseFloat(containerStyle.paddingTop) || 4;
                paddingBottom = parseFloat(containerStyle.paddingBottom) || 4;
            }
...
function checkOrientation() {
    ...
    if (!portraitWarning || !mainContainer) {
        return;
    }
    
    // 檢測是否為手機版（寬度 <= 768px）且為豎屏
