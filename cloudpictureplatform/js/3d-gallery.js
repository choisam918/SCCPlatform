...
async function init3DGallery() {
    // 移動端檢測（在函數頂部定義，以便在整個函數中使用）
    const isMobileDevice = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || 
                    (window.innerWidth <= 768);
    
    try {
        // 移動端WebGL支持檢測（僅檢測，不阻止使用）
        if (isMobileDevice) {
            // 檢測WebGL支持（僅記錄，不阻止）
            const testCanvas = document.createElement('canvas');
            const gl = testCanvas.getContext('webgl') || testCanvas.getContext('experimental-webgl');
            if (!gl) {
                console.warn('移動端不支持WebGL，將嘗試繼續初始化');
                // 不阻止，讓系統繼續嘗試初始化，如果失敗會自動降級
            } else {
                console.log('移動端WebGL支持檢測通過');
            }
        }
        ...
        // 移動端優化：使用window.innerWidth/innerHeight作為後備
        let canvasWidth = canvas.clientWidth || canvas.width || window.innerWidth || 800;
        let canvasHeight = canvas.clientHeight || canvas.height || window.innerHeight || 600;
        console.log('Canvas尺寸:', canvasWidth, 'x', canvasHeight);
        
        // 如果canvas尺寸為0，設置默認值（移動端優化）
        if (canvasWidth === 0 || canvasHeight === 0) {
            console.warn('Canvas尺寸為0，使用默認尺寸');
            // 移動端使用視窗尺寸
            canvasWidth = window.innerWidth || 800;
            canvasHeight = window.innerHeight || 600;
        ...
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || 
                        (window.innerWidth <= 768);
        console.log('設備類型:', isMobile ? '移動端' : '桌面端');
        ...
        const rendererOptions = {
            canvas: canvas,
            antialias: !isMobile, // 移動端關閉抗鋸齒以提升性能
            alpha: false, // 不透明背景
            powerPreference: 'high-performance', // 優先使用高性能GPU
            failIfMajorPerformanceCaveat: false, // 即使性能較差也繼續
            preserveDrawingBuffer: false // 移動端不需要保留繪製緩衝
        };
        
        try {
            renderer = new THREE.WebGLRenderer(rendererOptions);
        } catch (error) {
            console.warn('WebGL渲染器創建失敗，嘗試降級選項:', error);
            // 降級：移除高級選項
        ...
        // 移動端優化：減少陰影質量以提升性能
        renderer.shadowMap.enabled = !isMobile; // 移動端關閉陰影
        renderer.setClearColor(0x1a1a2a, 1); // 深藍黑色背景，與展廳風格一致
        renderer.shadowMap.type = isMobile ? THREE.BasicShadowMap : THREE.PCFSoftShadowMap; // 移動端使用基本陰影
        ...
        try {
            ...
            controls.dampingFactor = isMobile ? 0.12 : 0.08; // 移動端增加阻尼以減少抖動
            ...
            // 允許360度完整水平旋轉
            controls.maxAzimuthAngle = Math.PI; // 180度（向右），完整旋轉
            controls.minAzimuthAngle = -Math.PI; // -180度（向左），總共360度
            controls.enablePan = true; // 允許平移
            controls.panSpeed = isMobile ? 0.3 : 0.5; // 移動端降低平移速度
        ...
        const traditionalGalleryEl = document.getElementById('traditionalGallery');
        
        if (galleryContainerEl) galleryContainerEl.style.display = 'block';
        if (emptyStateEl) emptyStateEl.style.display = 'none';
        if (traditionalGalleryEl) traditionalGalleryEl.style.display = 'none';
        ...
        // 移動端優化：不拋出錯誤，讓上層自動降級到相冊模式
        // 這樣移動端即使3D初始化失敗，也能正常使用相冊模式
        if (isMobileDevice) {
            console.warn('移動端3D初始化失敗，將自動降級到相冊模式');
            // 不拋出錯誤，讓上層處理降級
        } else {
            throw error; // 桌面端仍然拋出錯誤以便調試
        }
    }
}
...
function createParticleSystem() {
    ...
    const particleMaterial = new THREE.PointsMaterial({
        size: isMobile ? 0.04 : 0.06, // 移動端減小粒子大小
        transparent: true,
        opacity: isMobile ? 0.3 : 0.4, // 移動端降低不透明度
        vertexColors: true,
...
function setupLighting() {
    ...
    mainLight.castShadow = !isMobile; // 移動端關閉陰影以提升性能
    // 移動端優化：降低陰影貼圖分辨率
        ...
        textureLoader.load(
            ...
            (texture) => {
                ...
                try {
                    ...
                    if (isMobile) {
                        // 移動端：限制紋理大小和質量
                        texture.minFilter = THREE.LinearFilter; // 移動端使用簡單過濾
                        texture.magFilter = THREE.LinearFilter;
                        texture.generateMipmaps = false; // 移動端關閉Mipmap以節省內存
                        texture.anisotropy = 1; // 移動端降低各向異性過濾
                    } else {
