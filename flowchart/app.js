...
function main() {
  ...
  // Undo/redo (diagram only: nodes/edges). Viewport pan/zoom is not part of history.
  const history = { stack: [], index: -1, max: 80 };
  function serializeDiagramOnly() {
    return {
  ...
  function loadDiagramOnly(data) {
    // Preserve viewport/tool; only replace nodes/edges.
    state.nodes.clear();
    state.edges.clear();
  ...
  function commitHistory() {
    const snap = serializeDiagramOnly();
    const s = JSON.stringify(snap);
    const cur = history.index >= 0 ? history.stack[history.index] : null;
    if (cur === s) return;
  ...
  function undoHistory() {
    ...
    history.index -= 1;
    const snap = JSON.parse(history.stack[history.index]);
    loadDiagramOnly(snap);
  ...
  function renderNodes() {
    ...
    for (const n of state.nodes.values()) {
      ...
      const isPrimary = state.selection.kind === "node" && state.selection.id === n.id;
      const isMulti = state.selectedNodeIds?.has(n.id);
      el.classList.toggle("is-selected", Boolean(isPrimary || isMulti));
  ...
  function withExportViewport(bounds, scale, fn) {
    ...
    try {
      // Re-measure bounds in SCREEN space including routed edges (loops can go outside node boxes).
      let minX = 0,
        minY = 0,
        maxX = bw * scale,
        maxY = bh * scale;
      ...
      for (const e of state.edges.values()) {
        ...
        if (!a || !b) continue;
        const aw = nodeAnchorPointForIndex(a, e.fromPort);
        const bw2 = nodeAnchorPointForIndex(b, e.toPort);
        const p0 = worldToScreen(aw);
        const p3 = worldToScreen(bw2);
        const pts = screenPolylineForEdge(e, p0, p3, aw, bw2);
        for (const p of pts) {
          if (!p) continue;
      ...
      const pad = Math.max(24, 18 + 12 * Math.min(scale, 2)); // arrow + stroke safety
      // Shift so everything is within [0,0]..[W,H].
      state.viewport.panX += -minX + pad;
      state.viewport.panY += -minY + pad;
      const cssW = Math.max(1, (maxX - minX) + pad * 2);
      const cssH = Math.max(1, (maxY - minY) + pad * 2);
      fn({ cssW, cssH });
    } finally {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    const isA = String(e.key || "").toLowerCase() === "a";
    if (!isA) return;
    if (!(e.ctrlKey || e.metaKey)) return;
    e.preventDefault();
    state.selectedNodeIds.clear();
    for (const id of state.nodes.keys()) state.selectedNodeIds.add(id);
    const first = state.nodes.keys().next().value || null;
    state.selection = first ? { kind: "node", id: first } : { kind: "none", id: null };
    render();
  });
  ...
  // Toggle palette (basic flowchart) panel: Ctrl/Cmd + D
  document.addEventListener("keydown", (e) => {
    const t = e.target;
    if (t && (t.tagName === "INPUT" || t.tagName === "TEXTAREA" || t.isContentEditable)) return;
    const key = String(e.key || "").toLowerCase();
  ...
  function normalizeMultiSelectionPrimary() {
    if (state.selection.kind !== "node") return;
    const id = state.selection.id;
    if (id && state.selectedNodeIds.size) {
      if (!state.selectedNodeIds.has(id)) state.selectedNodeIds.add(id);
    }
  }
  ...
  function deleteSelection() {
    ...
    // Node(s)
    normalizeMultiSelectionPrimary();
    const ids = state.selectedNodeIds.size
      ? Array.from(state.selectedNodeIds)
      : state.selection.kind === "node" && state.selection.id
        ? [state.selection.id]
        : [];
  ...
  state.selectedNodeIds.add(firstId);
  commitHistory();
  ...
  viewport.addEventListener(
    ...
    if (targetNode) {
      const id = targetNode.dataset.id;
        ...
        // Multi-select: Shift+Click toggles node membership.
        if (e.shiftKey) {
          if (state.selectedNodeIds.has(id)) state.selectedNodeIds.delete(id);
          else state.selectedNodeIds.add(id);
          const stillAny = state.selectedNodeIds.size > 0;
          state.selection = stillAny ? { kind: "node", id } : { kind: "none", id: null };
        } else {
          // If we're clicking inside an existing multi-selection, keep it (so group drag works after Ctrl+A).
          if (state.selectedNodeIds.size > 1 && state.selectedNodeIds.has(id)) {
            state.selection = { kind: "node", id };
          } else {
            state.selectedNodeIds.clear();
            state.selectedNodeIds.add(id);
        ...
        if (state.tool === "select") {
          normalizeMultiSelectionPrimary();
          const ids = state.selectedNodeIds.size ? Array.from(state.selectedNodeIds) : [id];
          const startNodes = new Map();
          for (const sid of ids) {
            const sn = state.nodes.get(sid);
            if (sn) startNodes.set(sid, { x: sn.x, y: sn.y });
          }
        state.drag = {
            kind: "drag-node",
          id,
  ...
  viewport.addEventListener(
    ...
    const world = screenToWorld(screen);
      if (state.drag.kind === "drag-node") {
        ...
        if (!n) return;
        const dxw = world.x - state.drag.startWorld.x;
        const dyw = world.y - state.drag.startWorld.y;
        let nx = state.drag.startNode.x + dxw;
        let ny = state.drag.startNode.y + dyw;
        ...
        // Apply to group (multi-select) if present.
        const ids = Array.isArray(state.drag.ids) && state.drag.ids.length ? state.drag.ids : [state.drag.id];
        const startNodes =
          state.drag.startNodes instanceof Map
            ? state.drag.startNodes
            : new Map([[state.drag.id, state.drag.startNode]]);
        for (const id of ids) {
          const sn = startNodes.get(id);
          const nn = state.nodes.get(id);
          if (!sn || !nn) continue;
          let tx = sn.x + dxw;
          let ty = sn.y + dyw;
          if (id === state.drag.id) {
            // For the grabbed node, apply snapping & alignment.
            tx = nx;
            ty = ny;
          } else if (!e.shiftKey) {
            // Keep group aligned to grid when snapping is on.
            tx = snapToGrid(tx, 24);
            ty = snapToGrid(ty, 24);
          }
          nn.x = tx;
  ...
  window.addEventListener(
    ...
    if (state.drag?.kind === "drag-node" && state.drag.didMove) {
      commitHistory();
    }
    state.drag = null;
      state.guides = null;
