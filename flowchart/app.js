...
function main() {
  ...
  // Mobile browsers (especially iOS Safari) can change the visual viewport height
  // when the address bar collapses/expands; keep canvas/SVG hit area in sync.
  let resizeRaf = 0;
  ...
  // Pointer-based palette drag (mobile-friendly).
  ...
  function endPaletteDrag() {
    ...
    if (paletteDrag.ghost) paletteDrag.ghost.style.display = "none";
  ...
  function addNodeFromClientPoint(clientX, clientY, shape, tone, { noSnap }) {
    const rect = getViewportRect();
    const world = screenToWorld({ x: clientX - rect.left, y: clientY - rect.top });
    addNodeFromDrop({ ...world, noSnap }, shape, tone);
  }
  ...
  function onViewportPointerDown(e) {
    // Mobile browsers may report button=-1 for touch pointers.
    // Only enforce "left button" for mouse pointers.
  ...
  function onViewportPointerMove(e) {
    ...
    if (state.drag.kind === "pending-drag-node") {
      ...
      // Touch pointers are noisy; raise threshold to avoid accidental drags
      const threshold = state.drag.pointerType === "touch" ? 12 : 5;
  ...
  // Mobile reliability: use click for line-tool node picking.
  // Some mobile browsers synthesize clicks more consistently than pointerdown logic.
  function onLineToolNodePick(evt) {
    ...
    // Prevent the click from also triggering other handlers (e.g. viewport click).
    evt.preventDefault?.();
    evt.stopPropagation?.();
    ...
    addEdge(state.lineDraft.fromNodeId, id, { routing: state.edgeRouting });
    state.lineDraft.fromNodeId = null;
    state.drag = null;
    setSelection("edge", state.selection.id);
    setStatus("線：完成");
    render();
  }

  nodesLayer.addEventListener("click", onLineToolNodePick, true);

  viewport.addEventListener(
    "click",
    (e) => {
      if (state.tool !== "line") return;
      // Click empty space cancels draft
      const nodeEl = e.target.closest?.(".node");
      if (nodeEl) return;
      if (!state.lineDraft.fromNodeId) return;
      state.lineDraft.fromNodeId = null;
      state.drag = null;
      setStatus("線：已取消");
      render();
  ...
  // Palette pointer drag handlers (capture on the palette container).
  flowPresets?.addEventListener(
    "pointerdown",
    (e) => {
      const btn = e.target.closest?.(".flowbtn");
      if (!btn) return;
      // Only do pointer-drag for touch/pen; mouse users can still use HTML5 drag.
  ...
  flowPresets?.addEventListener(
    ...
    (e) => {
      ...
      setGhostAt(e.clientX, e.clientY, (paletteDrag.ghost?.textContent || "").trim());
      viewport.classList.toggle("viewport--drop-target", isClientInViewport(e.clientX, e.clientY));
      e.preventDefault();
    },
    { passive: false }
  );
  ...
  flowPresets?.addEventListener(
    ...
    (e) => {
      ...
      endPaletteDrag();
      if (inView) {
        addNodeFromClientPoint(e.clientX, e.clientY, shape, tone, { noSnap: e.shiftKey });
      }
      e.preventDefault();
    },
    { passive: false }
  ...
  // visualViewport exists on most mobile browsers; helps keep geometry correct.
