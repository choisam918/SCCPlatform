...
function manhattanByPorts(p0, p3, fromPort, toPort) {
  ...
  // Bottom/top ports: prefer leaving in the port direction first (side targets only: 1 / 3).
  if (fp === 2 && (tp === 1 || tp === 3)) {
    const y1 = a.y + 32;
    if (Math.abs(b.x - a.x) < 0.5) return [a, b];
    return [a, { x: a.x, y: y1 }, { x: b.x, y: y1 }, b];
  }
  if (fp === 0 && (tp === 1 || tp === 3)) {
    const y1 = a.y - 32;
    if (Math.abs(b.x - a.x) < 0.5) return [a, b];
...
function main() {
  ...
  function drawEdges() {
    ...
    if (state.tool === "line" && state.lineDraft.fromNodeId && state.drag?.kind === "line-preview") {
      ...
      if (from) {
        const aw = nodeAnchorPointForIndex(from, state.lineDraft.fromPort ?? 0);
        const p0 = worldToScreen(aw);
        const p3 = state.drag.screen;
        const firstAxis =
          state.lineDraft.fromPort === 1 || state.lineDraft.fromPort === 3
            ? "h"
            : state.lineDraft.fromPort === 0 || state.lineDraft.fromPort === 2
              ? "v"
              : null;
        const dx = p3.x - p0.x;
        const dy = p3.y - p0.y;
        const backward = p3.y < p0.y - 0.5;
        const loopOutside = backward && Math.abs(dx) < 90 && Math.abs(dy) > 140;
        const lp = state.lineDraft.fromPort === 3 ? "left" : state.lineDraft.fromPort === 1 ? "right" : null;
        const pts = edgePolylinePoints(p0, p3, state.drag.shiftKey ? "straight" : state.edgeRouting, {
          firstAxis,
          loopSide: lp,
          loopOutside,
          fromPort: state.lineDraft.fromPort,
          toPort: null,
        });
        ctx.save();
        ctx.setLineDash([6, 6]);
